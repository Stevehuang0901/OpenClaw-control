...
export function EventFeed({ agents, events, messages }: EventFeedProps) {
  ...
  return (
    <section className="panel p-5">
      <p className="pixel-label">Comms</p>
      <h2 className="mt-2 text-2xl font-bold text-ink">Realtime log</h2>
        ...
        <div>
          ...
          <div className="mt-3 space-y-3">
            {messages.slice(0, 8).map((message) => (
              <article
                key={message.id}
                className="rounded-none border-2 border-ink bg-white/60 p-3"
              >
                ...
                <p className="mt-2 text-sm leading-relaxed text-ink/75">
                  {truncate(message.payload, 140)}
                </p>
                {message.reasoning ? (
                  <p className="mt-2 text-xs italic text-ink/55">
                    {truncate(message.reasoning, 120)}
                  </p>
                ) : null}
          ...
          <div className="mt-3 space-y-2">
            {events.slice(0, 10).map((event, index) => (
              <div
                ...
                className="flex items-center justify-between gap-3 rounded-none border-2 border-ink/15 bg-paper/60 px-3 py-2 text-sm text-ink/75"
              >
