import { useEffect, useState } from "react";

import { formatDateTime, formatNumber, truncate } from "../lib/format";
import type {
  ClawHubCatalogItem,
  ClawHubCatalogResponse,
  OpenClawSkillsSnapshot,
  SkillInstallResult
} from "../types/contracts";

const emptySkills: OpenClawSkillsSnapshot = {
  workspaceDir: "",
  managedSkillsDir: "",
  skills: []
};

const quickQueries = ["calendar", "github", "automation", "finance"];

export function SkillMarketplacePanel() {
  const [skills, setSkills] = useState<OpenClawSkillsSnapshot>(emptySkills);
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<ClawHubCatalogItem[]>([]);
  const [loadingSkills, setLoadingSkills] = useState(true);
  const [searching, setSearching] = useState(false);
  const [installingSlug, setInstallingSlug] = useState<string | null>(null);
  const [statusMessage, setStatusMessage] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const customSkills = skills.skills.filter((skill) => !skill.bundled);
  const readySkills = skills.skills.filter((skill) => skill.eligible).length;

  useEffect(() => {
    void refreshLocalSkills();
  }, []);

  const refreshLocalSkills = async () => {
    setLoadingSkills(true);

    try {
      const response = await fetch("/api/skills/openclaw");
      if (!response.ok) {
        throw new Error("Failed to load local skills.");
      }

      const data = (await response.json()) as OpenClawSkillsSnapshot;
      setError(null);
      setSkills(data);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "Unable to load skills.");
    } finally {
      setLoadingSkills(false);
    }
  };

  const runSearch = async (nextQuery: string) => {
    const trimmed = nextQuery.trim();
    setQuery(trimmed);
    setSearching(true);
    setError(null);

    try {
      if (!trimmed) {
        setResults([]);
        return;
      }

      const response = await fetch(
        `/api/skills/catalog?query=${encodeURIComponent(trimmed)}&limit=8`
      );
      if (!response.ok) {
        throw new Error("ClawHub search failed.");
      }

      const data = (await response.json()) as ClawHubCatalogResponse;
      setResults(data.items);
    } catch (searchError) {
      setError(searchError instanceof Error ? searchError.message : "Search failed.");
    } finally {
      setSearching(false);
    }
  };

  const installSkill = async (item: ClawHubCatalogItem) => {
    setInstallingSlug(item.slug);
    setError(null);
    setStatusMessage(null);

    try {
      const response = await fetch("/api/skills/install", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          slug: item.slug,
          version: item.latestVersion
        })
      });

      if (!response.ok) {
        const data = (await response.json().catch(() => ({ error: "Install failed." }))) as {
          error?: string;
        };
        throw new Error(data.error ?? "Install failed.");
      }

      const result = (await response.json()) as SkillInstallResult;
      setStatusMessage(result.message);
      await refreshLocalSkills();

      if (query.trim()) {
        await runSearch(query);
      }
    } catch (installError) {
      setError(installError instanceof Error ? installError.message : "Install failed.");
    } finally {
      setInstallingSlug(null);
    }
  };

  return (
    <section className="panel p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="pixel-label">Skill Market</p>
          <h2 className="mt-2 text-2xl font-bold text-ink">
            One-click ClawHub installs
          </h2>
        </div>
        <button
          type="button"
          className="rounded-none border-2 border-ink bg-paper px-3 py-2 text-xs uppercase tracking-[0.18em] text-ink shadow-pixel"
          onClick={() => void refreshLocalSkills()}
        >
          Refresh
        </button>
      </div>

      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <div className="rounded-none border-2 border-ink bg-paper/70 p-3 shadow-pixel">
          <p className="text-[10px] uppercase tracking-[0.22em] text-ink/55">
            Ready skills
          </p>
          <p className="mt-2 text-2xl font-bold text-ink">{formatNumber(readySkills)}</p>
          <p className="mt-2 text-xs text-ink/65">OpenClaw reports these as eligible now.</p>
        </div>
        <div className="rounded-none border-2 border-ink bg-paper/70 p-3 shadow-pixel">
          <p className="text-[10px] uppercase tracking-[0.22em] text-ink/55">
            Custom installs
          </p>
          <p className="mt-2 text-2xl font-bold text-ink">
            {formatNumber(customSkills.length)}
          </p>
          <p className="mt-2 text-xs text-ink/65">
            Installed into {truncate(skills.managedSkillsDir || "~/.openclaw/skills", 42)}
          </p>
        </div>
      </div>

      <form
        className="mt-5 grid gap-3"
        onSubmit={(event) => {
          event.preventDefault();
          void runSearch(query);
        }}
      >
        <div className="flex flex-col gap-3 sm:flex-row">
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            className="flex-1 rounded-none border-2 border-ink bg-white/80 px-4 py-3 text-sm text-ink shadow-pixel outline-none transition focus:border-teal"
            placeholder="Search ClawHub skills like calendar, github, automation..."
          />
          <button
            type="submit"
            className="rounded-none border-2 border-ink bg-coral px-5 py-3 text-sm font-bold uppercase tracking-[0.2em] text-paper shadow-pixel"
            disabled={searching}
          >
            {searching ? "Searching..." : "Search"}
          </button>
        </div>
        <div className="flex flex-wrap gap-2">
          {quickQueries.map((suggestion) => (
            <button
              key={suggestion}
              type="button"
              className="rounded-none border-2 border-ink/20 bg-paper px-2 py-1 text-[10px] uppercase tracking-[0.18em] text-ink/70"
              onClick={() => void runSearch(suggestion)}
            >
              {suggestion}
            </button>
          ))}
        </div>
      </form>

      {statusMessage ? (
        <div className="mt-4 rounded-none border-2 border-teal bg-teal/10 px-4 py-3 text-sm text-teal">
          {statusMessage}
        </div>
      ) : null}

      {error ? (
        <div className="mt-4 rounded-none border-2 border-coral bg-coral/10 px-4 py-3 text-sm text-coral">
          {error}
        </div>
      ) : null}

      <div className="mt-5">
        <p className="text-xs uppercase tracking-[0.25em] text-ink/55">
          Installed custom skills
        </p>
        <div className="mt-3 space-y-3">
          {loadingSkills ? (
            <div className="rounded-none border-2 border-dashed border-ink/20 p-4 text-sm text-ink/60">
              Loading OpenClaw skills...
            </div>
          ) : null}

          {!loadingSkills && customSkills.length === 0 ? (
            <div className="rounded-none border-2 border-dashed border-ink/20 p-4 text-sm text-ink/60">
              No extra skills installed yet. Search ClawHub below and install one.
            </div>
          ) : null}

          {customSkills.map((skill) => (
            <article
              key={skill.name}
              className="rounded-none border-2 border-ink/15 bg-white/60 p-3"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-bold text-ink">{skill.name}</p>
                  <p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-ink/50">
                    {skill.source}
                  </p>
                </div>
                <span
                  className={`rounded-none border-2 border-ink px-2 py-1 text-[10px] uppercase tracking-[0.16em] ${
                    skill.eligible ? "bg-mint/35 text-ink" : "bg-brass/20 text-brass"
                  }`}
                >
                  {skill.eligible ? "ready" : "needs setup"}
                </span>
              </div>
              <p className="mt-3 text-sm text-ink/70">{truncate(skill.description, 140)}</p>
            </article>
          ))}
        </div>
      </div>

      <div className="mt-5">
        <p className="text-xs uppercase tracking-[0.25em] text-ink/55">
          Search results
        </p>
        <div className="mt-3 space-y-3">
          {!searching && query.trim() && results.length === 0 ? (
            <div className="rounded-none border-2 border-dashed border-ink/20 p-4 text-sm text-ink/60">
              No catalog matches for "{query}".
            </div>
          ) : null}

          {!query.trim() ? (
            <div className="rounded-none border-2 border-dashed border-ink/20 p-4 text-sm text-ink/60">
              Search ClawHub to fetch installable skills, then install them into your
              OpenClaw managed skills directory.
            </div>
          ) : null}

          {results.map((item) => (
            <article
              key={item.slug}
              className="rounded-none border-2 border-ink bg-paper/75 p-4 shadow-pixel"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-lg font-bold text-ink">{item.displayName}</p>
                  <p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-ink/50">
                    {item.slug}
                    {item.ownerHandle ? ` by ${item.ownerHandle}` : ""}
                  </p>
                </div>
                <button
                  type="button"
                  className="rounded-none border-2 border-ink bg-teal px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-paper shadow-pixel disabled:cursor-not-allowed disabled:bg-slate"
                  disabled={item.installed || installingSlug === item.slug}
                  onClick={() => void installSkill(item)}
                >
                  {item.installed
                    ? "Installed"
                    : installingSlug === item.slug
                      ? "Installing..."
                      : "Install"}
                </button>
              </div>

              <p className="mt-3 text-sm leading-relaxed text-ink/75">
                {truncate(item.summary, 180)}
              </p>

              <div className="mt-4 grid grid-cols-2 gap-2 text-xs uppercase tracking-[0.16em] text-ink/55">
                <span>Version {item.latestVersion ?? "--"}</span>
                <span>Score {item.score?.toFixed(3) ?? "--"}</span>
                <span>Downloads {formatNumber(item.downloads ?? 0)}</span>
                <span>Stars {formatNumber(item.stars ?? 0)}</span>
                <span>Installs {formatNumber(item.installsCurrent ?? 0)}</span>
                <span>Updated {formatDateTime(item.updatedAt)}</span>
              </div>
            </article>
          ))}
        </div>
      </div>
    </section>
  );
}
